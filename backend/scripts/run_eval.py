...
def main() -> None:
    parser = argparse.ArgumentParser(description="RAG vs NoRAG automated evaluation")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output directory. Default: backend/outputs/eval/{run_id}",
    )
    args = parser.parse_args()

    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir) if args.output_dir else Path("backend/outputs/eval") / run_id
